  ...
  try {
    ...
    if (result) {
      ...
      designStore.setGenerateError(
        "Не удалось перегенерировать дизайн. Попробуйте ещё раз.",
      );
    }
  } finally {
...
<template>
  <div v-if="hasDesign && designStore.current" class="flex flex-1 flex-col">
    ...
    <main class="flex-1 overflow-y-auto px-5 pt-4 pb-28">
      ...
      >
        <div
          v-if="designStore.generateError"
          class="mt-4 rounded-2xl bg-error-50 px-4 py-3"
        >
          <p class="text-sm font-medium text-error-700">
            {{ designStore.generateError }}
          </p>
        </div>
      </Transition>
    ...
    >
      <div class="flex gap-3">
        <AppButton
          ...
          size="lg"
          class="flex-1"
          :loading="designStore.isRegenerating"
          @click="regenerate"
        >
