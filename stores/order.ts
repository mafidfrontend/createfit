...
export const useOrderStore = defineStore('createfit-order', {
  state: (): { draft: OrderDraft; createdOrder: CreatedOrder | null; error: string } => {
    const base: OrderDraft = { customer: { ...emptyCustomer }, product: null, fabric: null, design: null, size: null, paymentMethod: null, paymentStatus: 'pending', delivery: { ...emptyDelivery }, subtotal: 0, deliveryPrice: DELIVERY_PRICE, totalPrice: DELIVERY_PRICE, manufacturingDays: MANUFACTURING_DAYS }
    ...
    if (persisted) {
      ...
      if (persisted.delivery) base.delivery = { ...base.delivery, ...persisted.delivery }
      if (persisted.paymentMethod) base.paymentMethod = persisted.paymentMethod
      base.subtotal = calculateSubtotal(base.product, base.fabric, base.design?.additionalPrice ?? 0)
      base.totalPrice = calculateTotal(base.product, base.fabric, base.design?.additionalPrice ?? 0, base.delivery.price)
    }
  ...
  actions: {
    ...
    setProduct(product: Product): void { this.draft.product = product; this.recalculate(); this.persist() },
    setFabric(fabric: Fabric): void { this.draft.fabric = fabric; this.recalculate(); this.persist() },
    setDesign(design: Design): void { this.draft.design = design; this.recalculate(); this.persist() },
    setSize(size: SizeSelection): void { this.draft.size = size; this.persist() },
    setPaymentMethod(method: PaymentMethod): void { this.draft.paymentMethod = method; this.persist() },
    setDelivery(delivery: Partial<Delivery>): void { this.draft.delivery = { ...this.draft.delivery, ...delivery }; this.recalculate(); this.persist() },
    ...
    persist(): void { savePersistedDraft(this.draft) },
