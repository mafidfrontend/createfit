    ...
    <div v-else class="space-y-4">
      <div v-for="order in orders" :key="order.id" class="rounded-2xl border border-line bg-white p-5">
        <div class="flex items-center justify-between">
          <span class="text-sm font-bold text-sage">Заказ {{ order.order_number }}</span>
          <span class="rounded-full bg-mint px-3 py-1 text-xs font-bold text-sage">
        ...
        <div class="mt-4 space-y-2 text-sm">
          <div class="flex justify-between gap-4">
            ...
            <b class="text-right">{{ order.product?.name }} <span v-if="order.size">({{ order.size }})</span></b>
          </div>
          <div class="flex justify-between gap-4">
          ...
          <div class="flex justify-between gap-4">
            ...
            <b class="text-right">{{ order.design?.existingDesignName || 'Свой дизайн' }}</b>
          </div>
          <div class="flex justify-between gap-4">
