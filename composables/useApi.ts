...
function mapErrorMessage(status: number, fallback: string): string {
  switch (status) {
    ...
    case 401:
      return "Не удалось авторизоваться. Откройте приложение через Telegram.";
    case 403:
...
async function request<T>(
  ...
  try {
    ...
    if (import.meta.dev)
      console.log(`[api] ${options.method ?? "GET"} ${path} failed`, err);
...
export function useApi() {
  return {
    ...
    createOrder(
      data: ApiCreateOrderRequest,
      idempotencyKey?: string,
    ): Promise<ApiCreateOrderResponse> {
      const key =
        idempotencyKey ?? (import.meta.client ? crypto.randomUUID() : "");

      // Telegram initData'ni olib, body'ga qo'shamiz
      const { getInitData } = useTelegram();
      const initData = getInitData();
